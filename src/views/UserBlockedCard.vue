<template>
  <div class="blocked-cards">
    <v-row class="d-flex justify-space-between" no-gutters>
      <v-col v-for="card in cardList" :key="card.id" class="" cols="12" sm="5">
        <v-card :elevation="3" class="my-5 pa-3">
          <div class="d-flex justify-space-between align-center">
            <span>
              ...
              <p class="grey--text darken-2 ma-0">
                <v-avatar color="grey darken-2" size="8"> </v-avatar>
                {{ card.budget_name }}
              </p>
            </span>
            <span>
          ...
          <div class="d-flex justify-space-between align-center">
            <v-btn elevation="2" outlined x-small>
              {{ card.card_type }}
            </v-btn>
          ...
          <div>
            <div class="d-flex justify-space-between align-center">
              <span>
                <p class="ma-0">
                  <v-avatar color="pink lighten-3" size="10"> </v-avatar>
                  Spent
                </p>
              </span>
              <span> {{ card.spent.value }} {{ card.spent.currency }} </span>
            </div>
            <div class="d-flex justify-space-between align-center">
              <span>
                <p class="ma-0">
    ...
    <div
      ...
      v-observe-visibility="visibilityChangedScrolledToBottom"
    ></div>
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    async fetchData() {
      try {
        let cardData = await await axios.get(
          baseAPI + `?status=blocked&_page=${this.page}`
        );
